...
let selectedProducts = products.filter(product => product.price >= 50);
let productNames = selectedProducts.map(product => product.name);
let totalPrice = selectedProducts.reduce((total, product) => total + product.price, 0);

console.log(`Productos seleccionados: ${productNames}`);
