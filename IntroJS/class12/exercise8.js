...
for (let i =0; i < matriz.length; i++) {
    for (let j = 0; j < matriz[i].length; j++) {
        if (i === j) {
            redSum += matriz[i][j];
            redDiagonal.push(matriz[i][j]);
        } else if (i + j === matriz.length - 1) {
            greenSum += matriz[i][j];
            greenDiagonal.push(matriz[i][j]);
        }   
    }
}

console.log(`Los valores de la diagonal roja son: ${redDiagonal} y su suma es: ${redSum}`);
console.log(`Los valores de la diagonal verde son: ${greenDiagonal} y su suma es: ${greenSum}`);