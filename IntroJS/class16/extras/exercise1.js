...
let names = ['Lucía', 'Ana', 'María', 'Luis', 'José', 'Pablo'];

let shortNames = names.filter(name => name.length < 5);
let upperCaseNames = shortNames.map(name => name.toUpperCase());
let resultPhrase = "Los nombres seleccionados son: " + upperCaseNames.join(", ");
